...
const accountFormSchema = z.object({
  ...
  subtype: z.enum(["checking", "savings", "money_market", "investment", "other_asset", "credit_card", "mortgage", "student_loan", "auto_loan", "line_of_credit", "other_debt"]),
...
export default function AccountsPage() {
  ...
  const allAccounts = Array.isArray(accounts) ? accounts.map(account => ({
    id: account.id,
    name: account.name,
    type: account.type === "ASSET" ? "Asset" : "Debt",
    accountType: account.subtype.charAt(0).toUpperCase() + account.subtype.slice(1).replace('_', ' '),
    apr: account.aprApy ? parseFloat(account.aprApy) : null,
    dueDate: account.paymentDueDay || null,
